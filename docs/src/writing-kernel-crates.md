...
## Native OpenCL vector types (`spirv_std::cl`)

For cases where glam's `Vec*`/`IVec*`/`UVec*` family doesn't fit (you
need width 8 or 16, the full integer family at every scalar width, or
the OpenCL `doubleN` ABI), use `spirv_std::cl::*`:

```rust
use spirv_std::cl::{self, s};

#[spirv(kernel)]
pub fn pack_pixels(
    #[spirv(global_invocation_id)] id: USizeVec3,
    #[spirv(cross_workgroup)] data: &mut [cl::UChar16],
) {
    let v = data[id.x];
    // OpenCL `sN` swizzle (hex digit per component, widths 1..=16):
    let lo = s!(v, s0123);  // UChar4 of the first 4 components
    // Letter swizzle (xyzw, widths 1..=4):
    let xy = s!(lo, xy);    // UChar2
    // Named-group swizzles return half the source width:
    let upper = v.hi();     // UChar8 of components 8..16
    let _ = (xy, upper);
}
```

What `cl::*` adds over glam:

- **Widths 8 and 16** for every scalar type (`Float8`, `Int16`,
  `UChar16`, `Double8`, …). Requires the `Vector16` capability — added
  by default for OpenCL targets.
- **Full integer family** at every width (`Char`/`UChar`/`Short`/
  `UShort`/`Int`/`UInt`/`Long`/`ULong`). Signed-aware codegen picks
  `OpSDiv`/`OpSRem` for signed types and `OpUDiv`/`OpURem` for unsigned.
- **OpenCL `doubleN` ABI** (`Double3` is 32-byte aligned, `Double4`
  is 32-byte, `Double8` is 64-byte, etc.) — matches what
  `<CL/cl_platform.h>` defines, so kernels can interop with host
  `cl_doubleN` buffers without padding mismatches.
- **`s!` swizzle macro** with letter form (`xyzw`) and OpenCL hex
  form (`s0`..`sf`); single-index extracts use `OpCompositeExtract`,
  multi-index uses one `OpVectorShuffle` with literal indices.
- **Named-group swizzles** `lo()`/`hi()`/`even()`/`odd()` for widths
  2/4/8/16 (width 3 is intentionally not supported — the OpenCL spec
  defines them as "operate as if width 4 with undef component", which
  is too easy to misuse; use `s!(v3, sN)` instead).

Operators (`+`, `-`, `*`, `/`, `Mul<Scalar>`) emit native vector
SPIR-V instructions, not per-component scalarisation. The same code
runs on the host (CPU fallback impls are provided for every operator
and swizzle), so `cl::Float3` works in `cpu_driver` for reference
computation.

The `arch::opencl_std::*` math functions accept `cl::*` types via
the same `FloatOrFloatVector` / `*IntegerOrInteger*Vector` traits
used for glam — `ocl::sqrt(cl::Double8)` and `ocl::s_min(cl::Char16)`
work the same way.
...
OpenCL images are kernel parameters typed as `&Image<...>` (read) or
`&mut Image<...>` (write). The codegen emits `OpTypeImage` with an
`AccessQualifier` chosen from the parameter's mutability:

- `&Image<...>` → `ReadOnly`
- `&mut Image<...>` → `WriteOnly` (OpenCL 1.2)
- `&mut Image<...>` with the `ImageReadWrite` capability → `ReadWrite`
  (OpenCL 2.0+; required when one kernel both reads and writes the
  same image)

OpenCL 1.2 supports both read-only and write-only images, so a single
image can be read by one kernel and written by another in the same
program without `ImageReadWrite`.
...
pub fn copy_image(
    #[spirv(global_invocation_id)] id: USizeVec3,
    src: &StorageImage2d,           // ReadOnly  on OpenCL 1.2
    dst: &mut StorageImage2d,       // WriteOnly on OpenCL 1.2
) {
    let pixel: glam::Vec4 = src.read(id.xy().as_ivec2());
    dst.write(id.xy().as_ivec2(), pixel);
}
```

For sampled reads (hardware filtering / wrap modes), pass a
`&Sampler` argument and use `Image::sample_by_lod`. The implicit-LOD
forms (`sample`/`sample_bias`) require fragment-shader derivatives
and are therefore not valid in Kernel mode — `sample_by_lod` with
an explicit LOD (often `0.0`) is the only sampling form available.

```rust
use spirv_std::image::{Image, sampler::Sampler};

#[spirv(kernel)]
pub fn upscale(
    #[spirv(global_invocation_id)] id: USizeVec3,
    src: &Image2d,                  // sampled image (sampled=true)
    sampler: &Sampler,
    dst: &mut StorageImage2d,
) {
    let uv = id.xy().as_vec2() / glam::Vec2::new(16.0, 16.0);
    let pixel: glam::Vec4 = src.sample_by_lod(*sampler, uv, 0.0);
    dst.write(id.xy().as_ivec2(), pixel);
}
```

Kernels that take a `&Sampler` must run on a target where the
`LiteralSampler` capability is available; opt in via SpirvBuilder:
...
The `examples/shaders/kernel-image-shader/` and
`examples/shaders/kernel-sampler-shader/` crates plus the matching
sections in `examples/runners/opencl/src/main.rs` exercise both flows
end-to-end on a real OpenCL device.
