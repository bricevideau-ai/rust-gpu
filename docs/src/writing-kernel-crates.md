...
compiling and running a kernel using the `opencl3` crate with pocl.
...
- **Enums**: Default enum discriminants use `u8` which requires the
  `Int8` capability. Use `#[repr(u32)]` for enums in kernel code.
