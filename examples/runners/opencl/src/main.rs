...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let path_to_crate =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../shaders/kernel-shader");
    let compile_result =
        SpirvBuilder::new(path_to_crate, "spirv-unknown-opencl1.2").build()?;
    let spv_path = compile_result.module.unwrap_single();
    let spv_bytes = std::fs::read(spv_path)?;
    println!("Compiled kernel ({} bytes SPIR-V, {:?})",
        spv_bytes.len(), compile_start.elapsed());
    ...
    let n = data.len();
    println!("Input:   {} elements ({}..{})", n, src_range.start, src_range.end - 1);

    // ── 5. Upload to device ────────────────────────────────────────────
    let mut buffer = unsafe {
        Buffer::<u32>::create(&context, CL_MEM_READ_WRITE, n, ptr::null_mut())?
    };
    unsafe {
        queue
