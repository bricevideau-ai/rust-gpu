...
fn main() {
    ...
    let target_specs_dir = original_target_dir.join("compiletest-target-specs");
...
impl Runner {
    ...
    fn run_mode(&self, mode: Mode) {
            ...
            .flat_map(|env| VARIATIONS.iter().map(move |variation| (env, variation)))
        {
            ...
            let target_spec =
                TargetSpecVersion::target_arg(rustc_version, &target, &self.target_specs_dir)
                    .unwrap();
