...
/// Number of f32 outputs per work-item: 33 functions in the intersection of
...
/// calls (`a.sin()`, `a.powf(b)`, etc.). On Kernel these go through rust-gpu's
/// codegen intercept of `Float` trait / inherent f32 methods; on CPU they
/// dispatch to libm / system math via the standard f32 inherent methods.
/// Slot order matches `opencl_std_compute` so the difftest comparison is
/// element-wise.
pub fn rust_compute(a: f32, b: f32, c: f32, out: &mut [f32]) {
    let mut fi = 0usize;
...
/// `opencl_std::*` calls. On Kernel these lower directly to
/// `OpExtInst <OpenCL.std> *`; on CPU they use spirv-std's host fallbacks.
/// Slot order matches `rust_compute`.
pub fn opencl_std_compute(a: f32, b: f32, c: f32, out: &mut [f32]) {
    let mut fi = 0usize;
    ...
    out[fi] = opencl_std::clamp(a, 0.0f32, 1.0f32);
    fi += 1;
...
/// Build the three deterministic input vectors of length `N`. Ranges chosen
/// so every domain is valid: `a ∈ [0.1, 0.9]` (asin/acos/atanh-safe),
/// `b ∈ [1.0, 3.0]` (ln/sqrt/log/acosh-safe), `c ∈ [0.0, 1.0]` (FMA scale).
#[cfg(not(target_arch = "spirv"))]
