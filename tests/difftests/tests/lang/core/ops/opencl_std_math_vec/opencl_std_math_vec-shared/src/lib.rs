...
const _: () = assert!(
    ...
    "THREADS * WORKGROUPS * 4 must equal N (one Float4 per work item)",
);
...
fn write4(output: &mut [f32], offset: usize, v: Float4) {
    let a = v.to_array();
    output[offset] = a[0];
    output[offset + 1] = a[1];
    output[offset + 2] = a[2];
...
/// Run all `FUNCS` Float4 intrinsics into `vec_out` (length `4 * FUNCS`).
/// Single source compiled for SPIR-V Kernel and host CPU.
pub fn compute(va: Float4, vb: Float4, vc: Float4, vec_out: &mut [f32]) {
    let mut fi = 0usize;
    ...
    write4(
        ...
        opencl_std::clamp(va, Float4::splat(0.0), Float4::splat(1.0)),
    );
    fi += 1;
    ...
    write4(
        ...
        opencl_std::smoothstep(Float4::splat(0.0), Float4::splat(1.0), va),
    );
    fi += 1;
